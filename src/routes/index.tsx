...
import { Sparkles, Heart, Scissors, Gift, Instagram, Facebook, ShoppingBag, ArrowRight } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Ateliê de personalizados, decoração artesanal e papelaria criativa para festas inesquecíveis. Feito com carinho." },
    ...
    links: [
      ...
      { rel: "stylesheet", href: "https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,500;0,9..144,700;1,9..144,500&family=Quicksand:wght@400;500;600;700&display=swap" },
    ],
  }),
  component: Index,
});
...
function Balloon({ className, color, r = "0deg", delay = "0s" }: { className?: string; color: string; r?: string; delay?: string }) {
  return (
    <div
      ...
      aria-hidden
    >
      <div className="w-16 h-20 rounded-[50%] shadow-soft" style={{ backgroundColor: `var(--${color})` }} />
      <div className="mx-auto w-px h-14" style={{ backgroundColor: "var(--muted-foreground)", opacity: 0.4 }} />
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <header className="absolute top-0 inset-x-0 z-30">
        <nav className="max-w-7xl mx-auto flex items-center justify-between px-6 py-5">
          <a href="#top" className="flex items-center gap-2">
            <img src={logo} alt="MUNNO Arte e Festa" className="w-12 h-12 rounded-full shadow-soft" />
      ...
      <section id="top" className="relative pt-32 pb-24 md:pt-40 md:pb-32 bg-confetti">
        ...
        <Balloon className="top-72 right-[22%] hidden lg:block" color="blue-soft" r="-4deg" delay="0.6s" />
        <Balloon className="top-28 left-[40%] hidden lg:block" color="mint-soft" r="6deg" delay="2s" />
      ...
      <section id="sobre" className="relative py-24 md:py-32">
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-14 items-center">
          <div>
            <span className="text-xs font-bold tracking-[0.2em] text-primary uppercase">Sobre o ateliê</span>
            <h2 className="mt-4 text-3xl md:text-5xl font-bold leading-tight">
              Pequenos detalhes que viram <em className="text-gradient-warm not-italic">grandes lembranças</em>.
            </h2>
            <p className="mt-6 text-lg text-muted-foreground leading-relaxed">
              A MUNNO Arte e Festa cria personalizados únicos para aniversários, mesversários
              e momentos especiais. Cada detalhe é pensado com criatividade, delicadeza e muito amor.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              {["Aniversários", "Mesversários", "Chá de bebê", "Casamentos"].map((t) => (
                <span key={t} className="rounded-full bg-card border border-border px-4 py-1.5 text-sm shadow-soft">{t}</span>
              ))}
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="absolute -top-6 -right-6 w-20 h-20 rounded-full opacity-70 blur-2xl" style={{ backgroundColor: "var(--orange-soft)" }} />
          </div>
        </div>
      </section>

      {/* PORTFÓLIO */}
      <section id="portfolio" className="relative py-24 md:py-32" style={{ backgroundColor: "var(--cream)" }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto">
            <span className="text-xs font-bold tracking-[0.2em] text-primary uppercase">Portfólio</span>
          ...
          <div className="mt-14 grid md:grid-cols-3 gap-6 md:gap-8">
            {[
              { src: portfolio1, alt: "Mesa de festa personalizada em tons pastel", tag: "Festa Infantil" },
              { src: portfolio2, alt: "Caixinhas e lembrancinhas artesanais", tag: "Lembrancinhas" },
          ...
          <div className="mt-14 text-center">
            ...
            <p className="mt-4 text-sm text-muted-foreground">Veja o feed completo no nosso Instagram</p>
          </div>
        </div>
      </section>
      ...
      <section id="diferenciais" className="relative py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto">
            <span className="text-xs font-bold tracking-[0.2em] text-primary uppercase">Por que a MUNNO</span>
          ...
          <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: Sparkles, title: "Personalização exclusiva", desc: "Cada projeto é único, pensado para a sua história.", color: "pink-soft" },
              { icon: Scissors, title: "Feito artesanalmente", desc: "Cortado, montado e finalizado à mão com capricho.", color: "yellow-soft" },
              { icon: Heart, title: "Atendimento com carinho", desc: "Conversa próxima do começo ao dia da festa.", color: "blue-soft" },
              { icon: Gift, title: "Produtos criativos e únicos", desc: "Ideias originais para encantar quem você ama.", color: "mint-soft" },
              ...
              >
                ...
                <span className="absolute top-5 right-5 text-xs font-display text-muted-foreground/60">0{i + 1}</span>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-20">
        <div className="max-w-5xl mx-auto px-6">
          <div className="relative overflow-hidden rounded-[2.5rem] p-10 md:p-16 text-center shadow-pop"
               style={{ background: "linear-gradient(135deg, var(--pink-soft), var(--orange-soft) 60%, var(--yellow-soft))" }}>
            <Balloon className="-top-2 left-6 hidden md:block" color="blue-soft" r="-10deg" />
            <Balloon className="-top-2 right-6 hidden md:block" color="mint-soft" r="10deg" delay="1s" />
        ...
        <div className="max-w-7xl mx-auto px-6 py-14 grid gap-10 md:grid-cols-3 items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground tracking-widest uppercase">Arte e Festa</p>
            </div>
          </div>

          <p className="text-center text-sm text-muted-foreground italic">
            Feito com <Heart className="inline w-4 h-4 text-primary fill-primary" /> para momentos especiais.
          </p>
          ...
          <div className="flex md:justify-end items-center gap-3">
            ...
            <a href={SHOPEE} target="_blank" rel="noopener noreferrer" aria-label="Shopee"
               className="w-11 h-11 rounded-full bg-background border border-border flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition">
